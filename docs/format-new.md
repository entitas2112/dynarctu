...
## 2. Examples (DYNARCTU TKA Format)

The following examples represent the three standard question types used in the DYNARCTU evaluation system.

### A. Standard Multiple Choice (Pilihan Ganda Sederhana)
Requires the `Answer` key to exactly match the text of the correct option.
```qdf
ID: TKA_001
Type: PilihanGanda
Question: Dalam sistem EFI (Electronic Fuel Injection), komponen yang bertugas mendeteksi jumlah udara yang masuk ke dalam intake manifold adalah...
Answer: Mass Air Flow (MAF) Sensor
Explanation: >
  MAF sensor mengukur volume dan kepadatan udara yang masuk ke mesin 
  agar ECU bisa menentukan jumlah bahan bakar yang tepat.
Options:
  - Text: Throttle Position Sensor (TPS)
  - Text: Oxygen Sensor (O2 Sensor)
  - Text: Mass Air Flow (MAF) Sensor
  - Text: Crankshaft Position Sensor (CKP)
  - Text: Manifold Absolute Pressure (MAP) Sensor
---
```

### B. Multiple Choice Multiple Answers / MCMA (Pilihan Ganda Kompleks)
Does not use a root `Answer` key. Instead, every option requires an `IsCorrect` boolean flag (`true` or `false`).
```qdf
ID: TKA_002
Type: PilihanGandaKompleks
Question: Di antara pilihan berikut, manakah aplikasi yang sering digunakan di ekosistem Linux untuk memodifikasi atau mengoptimalkan audio/tampilan desktop?
Explanation: EasyEffects berfungsi untuk routing audio. Conky adalah sistem monitor untuk widget desktop kustom.
Options:
  - Text: EasyEffects
    IsCorrect: true
  - Text: Conky
    IsCorrect: true
  - Text: Microsoft DirectX
    IsCorrect: false
  - Text: PulseAudio Volume Control (pavucontrol)
    IsCorrect: true
---
```

### C. True/False Category (Benar / Salah)
Does not use a root `Answer` key. Options are presented as statements, and `IsCorrect` expects a literal `Benar` or `Salah` string.
```qdf
ID: TKA_003
Type: BenarSalah
Question: Tentukan Benar atau Salah pada setiap pernyataan berikut terkait makna Pasal 31 UUD NRI Tahun 1945!
Explanation: Negara menjamin pendidikan dasar, bukan membiayai perguruan tinggi secara otomatis untuk seluruh warganya.
Options:
  - Text: Pasal tersebut menunjukkan bahwa negara bertanggung jawab penuh atas pendidikan warga negaranya.
    IsCorrect: Benar
  - Text: Negara menjamin setiap warga negara untuk melanjutkan pendidikannya ke perguruan tinggi secara gratis.
    IsCorrect: Salah
  - Text: Pasal tersebut menjadi dasar hukum bahwa pendidikan merupakan hak setiap warga tanpa diskriminasi.
    IsCorrect: Benar
---
```
